...
def signup(request):
    if request.method == 'POST':
        # if request.data.get('email') == ADMIN_EMAIL:
        #     return JsonResponse({'message': 'این ایمیل در سامانه وجود دارد'}, status=status.HTTP_400_BAD_REQUEST)
...
def get_users_by_role(request):
    if request.method == "POST":
        ...
        if data is None or data.count() == 0:
            return JsonResponse({'message': 'چیزی یافت نشده است'}, safe=False, status=status.HTTP_404_NOT_FOUND)
...
def get_all_class(request):
    login_token = request.META.get('HTTP_AUTHORIZATION', None)
    login_token = uuid.UUID(login_token, version=4)
    if login_token:
        try:
            join_class = JoinedClass.objects.filter(user=UserProfile.objects.get(login_token=login_token)).all()
